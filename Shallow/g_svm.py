...
def training_loop(X, mc, loo, splits):
    ...
    for m in range(mc):
        ...
        for i, (train_idx, test_idx) in enumerate(cv):
            ...
            pd.DataFrame(accuracies).to_csv('GSVM/accuracies'+loo+splits +'.csv', sep=';', decimal=',', index=False, header=False)
            pd.DataFrame(labels).to_csv('GSVM/labels_'+loo+splits +'.csv', sep=';', decimal=',', index=False, header=False)
            pd.DataFrame(predicted).to_csv('GSVM/predicted_'+loo+splits+'.csv', sep=';', decimal=',', index=False, header=False)
